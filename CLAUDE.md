...
- **LLM provider abstraction** (`llm/`): `LlmProvider` ABC (`base.py`) with `OpenAiProvider`, `GeminiProvider`, `ClaudeProvider` implementations, selected via `llm/factory.py::get_llm_provider(name)`. Every provider implements `test_connection`, `list_models`, `generate_json` (schema-constrained), `generate_text`. `ClaudeProvider` has two auth modes on the same stored secret, auto-detected by prefix at call time via `llm/claude_cli.py::is_oauth_token` (no DB schema involved): a regular Anthropic API key (`sk-ant-api03-...`) goes through the HTTP `anthropic` SDK as before, while a Claude Pro/Max subscription OAuth token (`sk-ant-oat01-...`, generated by the user via `claude setup-token`) is routed through `llm/claude_cli.py`, which shells out to the `claude` CLI binary in non-interactive print mode (`claude -p`) — the only Anthropic-sanctioned way to spend subscription usage outside an interactive session; direct Messages API calls with an OAuth token are blocked server-side and against Anthropic's ToS. The CLI binary is installed into the Docker image via Anthropic's native `install.sh` (see `Dockerfile`) — `DISABLE_AUTOUPDATER`/`DISABLE_UPDATES` must only be set *after* that install step, since the installer treats them as an admin policy and silently no-ops the install (while still printing "Installation complete!") if they're already set. The `claude` binary also hard-refuses `--permission-mode bypassPermissions`/`--dangerously-skip-permissions` when it detects it's running as root, with no override flag — since the container itself still runs as root (unchanged, to avoid breaking existing users' `/data` volume permissions), `claude_cli.py::_drop_privileges_uid_gid` runs only the CLI subprocess as the unprivileged `claudecli` system user created in the `Dockerfile`, which is why the binary is relocated there to a world-readable path (`/opt/claude-code`) rather than left under root's install location.
