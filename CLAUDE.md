...
- **LLM provider abstraction** (`llm/`): `LlmProvider` ABC (`base.py`) with `OpenAiProvider`, `GeminiProvider`, `ClaudeProvider` implementations, selected via `llm/factory.py::get_llm_provider(name)`. Every provider implements `test_connection`, `list_models`, `generate_json` (schema-constrained), `generate_text`. All three providers authenticate with a regular provider API key (Anthropic's `sk-ant-api03-...` for Claude) over that provider's HTTP SDK — there is no CLI subprocess or OAuth/subscription auth path.
