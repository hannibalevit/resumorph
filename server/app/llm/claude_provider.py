...
class ClaudeProvider(LlmProvider):
    ...
    async def generate_text(self, api_key: str, model: str, system_prompt: str, user_prompt: str, max_tokens: int = 2000) -> str:
        ...
        response = await client.messages.create(model=model, max_tokens=max_tokens, temperature=0, system=system_prompt, messages=[{"role": "user", "content": user_prompt}])
    ...
    async def test_connection(self, api_key: str, model: str | None = None) -> dict[str, Any]:
        text = await self.generate_text(api_key, model or "claude-3-5-haiku-latest", "Reply with exactly: ok", "ok", 8)
